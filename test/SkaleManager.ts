    ...
    describe("when validator has delegated SKALE tokens", async () => {
        ...
        describe("when 16 nodes are in the system", async () => {
            ...
            it("should create 16 nodes & create & delete all types of schain", async () => {
                ...
                for (let j = 2; j < 8; j++) {
                    const typeOfScahin = 2;
                    let price = web3.utils.toBN(await schains.getSchainPrice(typeOfScahin, 5));
                    await skaleToken.send(
                        skaleManager.address,
                        price.toString(),
                        web3.eth.abi.encodeParameters(["uint", "uint8", "uint16", "string"], [
                            5, // lifetime
                            typeOfScahin, // type of schain
                            0, // nonce
                            "d1"+j.toString()]), // name
                        {from: developer});

                    price = web3.utils.toBN(await schains.getSchainPrice(typeOfScahin-1, 5));
                    await skaleToken.send(
                        skaleManager.address,
                        price.toString(),
                        web3.eth.abi.encodeParameters(["uint", "uint8", "uint16", "string"], [
                            5, // lifetime
                            typeOfScahin-1, // type of schain
                            0, // nonce
                            "d2"+j.toString()]), // name
                        {from: developer});
                    price = web3.utils.toBN(await schains.getSchainPrice(typeOfScahin, 5));
                    await skaleToken.send(
                        skaleManager.address,
                        price.toString(),
                        web3.eth.abi.encodeParameters(["uint", "uint8", "uint16", "string"], [
                    ...
                    // schain1[0].should.be.equal("d"+i.toString());
                    console.log((await schainsInternal.getNodesInGroup(web3.utils.soliditySha3("d"+j.toString()))).map(v => v.toNumber()))
                    for (let k = 0; k < 16; k++) {
                        console.log((await nodesContract.spaceOfNodes(k))[0].toNumber());
                    }

                    await skaleManager.deleteSchain("d1"+j.toString(), {from: developer});
                    await skaleManager.deleteSchain("d2"+j.toString(), {from: developer});
                    await skaleManager.deleteSchain("d3"+j.toString(), {from: developer});
            }
            for (let k = 0; k < 16; k++) {
                console.log((await nodesContract.spaceOfNodes(k))[0].toNumber());
            }
